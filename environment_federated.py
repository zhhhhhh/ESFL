...
#======================================= Start of training function ===========================================================#
    def participant_update(self, global_epoch, model, attack_type = 'no_attack', malicious_behavior_rate = 0, 
        ...
        # print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
        model = model.cpu()
        return model.state_dict(), peer_grad , model, np.mean(epochs_loss), attacked, t
...
#Test label prediction function    
    ...
    def run_experiment(self, attack_type = 'no_attack', malicious_behavior_rate = 0,
        ...
        cpu_runtimes = []
        # best_accuracy = 0.0
        # mapping = {'honest': 1, 'attacker': 0}

        #start training
        start_round = 0
        if resume:
            print('Loading last saved checkpoint..')
            checkpoint = torch.load('./checkpoints/'+ self.dataset_name + '_' + self.model_name + '_' + self.dd_type + '_'+ rule + '_'+ str(self.attackers_ratio) + '.t7')
        ...
        for epoch in tqdm_notebook(range(start_round, self.global_rounds)):
            ...
            elif rule == 'lfa_defender':
                cur_time = time.time()
                if self.dd_type != 'EXTREME_NON_IID':
                    global_weights = lfd.aggregate_mild(copy.deepcopy(simulation_model), copy.deepcopy(local_models), peers_types)
                else:
                    global_weights = lfd.aggregate_extreme(copy.deepcopy(simulation_model), copy.deepcopy(local_models), 
                    minimum_cluster_size = 2, peers_types = peers_types)
                cpu_runtimes.append(time.time() - cur_time)
                ##############################################################################################
            #Plot honest vs attackers
            # if attack_type == 'label_flipping':
                # plot_updates_components(local_weights, peers_types, epoch=epoch+1)   
                # plot_layer_components(local_weights, peers_types, epoch=epoch+1, layer = 'linear.weight')  
            ...
            savepath = './checkpoints/'+ self.dataset_name + '_' + self.model_name + '_' + self.dd_type + '_'+ rule + '_'+ str(self.attackers_ratio) + '.t7'
        ...
        savepath = './results/'+ self.dataset_name + '_' + self.model_name + '_' + self.dd_type + '_'+ rule + '_'+ str(self.attackers_ratio) + '.t7'
        torch.save(state,savepath)            
